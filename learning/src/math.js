...
export function binomialPMF(k,n,p) {
  if(!Number.isInteger(n)||n<0||!Number.isFinite(p)||p<0||p>1)throw new RangeError('試行回数は0以上の整数、確率は0から1です。');
  if(!Number.isInteger(k)||k<0||k>n)return 0;
  let coefficient=1;
  for(let i=1;i<=Math.min(k,n-k);i++)coefficient*=((n-i+1)/i);
  return coefficient*p**k*(1-p)**(n-k);
}
export function poissonPMF(k,lambda) {
  if(!Number.isFinite(lambda)||lambda<0)throw new RangeError('平均回数は0以上です。');
  if(!Number.isInteger(k)||k<0)return 0;
  let probability=Math.exp(-lambda);
  for(let i=1;i<=k;i++)probability*=lambda/i;
  return probability;
}
// Student's t density with three degrees of freedom.
export const studentT3PDF=x=>2/(Math.PI*Math.sqrt(3))*(1+x*x/3)**-2;
