...
function MethodLinks({ids,methodName}){return <span className="tree-methods">{(Array.isArray(ids)?ids:[ids]).map(id=><PageLink key={id} href={`#/statistics/method/${id}`}>{methodName||methods.find(item=>item.id===id).name} →</PageLink>)}</span>;}
...
export function SelectionChart(){return <Section title="選択チャートで、分かれ道を見る">
 <p>まずは<PageLink href="#/statistics/method/describe">記述統計・可視化</PageLink>や<PageLink href="#/statistics/method/crosstab">クロス集計</PageLink>でデータを見ます。そのうえで、下の分かれ道をたどってください。手法名を押すと、具体例と結果の読み方を開けます。</p>
 ...
 <p>一元・二元の「元」は、人数や画面の数ではなく、比べる要因の数です。二元配置以上をまとめて「多元配置」と呼ぶこともあります。これは候補を探すための地図です。<strong>同じ人を何度も測る場合や、クラス・施設のまとまりがある場合</strong>は、そのつながりも扱う必要があります。<PageLink href="#/statistics/method/mixed">混合効果モデル</PageLink>などを検討します。</p>
 <p>関係の仮説全体を調べる<PageLink href="#/statistics/method/sem">構造方程式モデリング（SEM）</PageLink>など、単純な枝分かれでは表しきれない手法もあります。候補の説明で、自分の問いやデータに合うかを確かめてください。</p>
 </Section>;}
